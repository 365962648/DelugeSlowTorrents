...
class Command(BaseCommand):
    ...
    def show_info(self, torrent_id, status, verbose=False):
        ...
        if status["state"] in ("Seeding", "Downloading"):
            ...
            s = "{!info!}Max Up/Down Speed: {!input!}%s/%s KiB/s" % (status["max_upload_speed"], status["max_download_speed"])
            if common.ftime(status["eta"]):
                s += " {!info!}ETA: {!input!}%s" % status["eta"]
        ...
        s += " {!info!}Raw(U/D/W/T): {!input!}%s/%s/%s/%s" % (status["total_uploaded"], status["total_done"], status["total_wanted"], status["total_size"])
        self.console.write(s)
        ...
        self.console.write("{!info!}Save path: {!input!}%s" % status["save_path"])
        
        s = "{!info!}Tracker status: {!input!}%s" % status["tracker_status"]
        s += " {!info!}Next Announce: {!input!}%s" % status["next_announce"]
        self.console.write(s)
